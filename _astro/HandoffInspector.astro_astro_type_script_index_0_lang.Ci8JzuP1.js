function H(o,t){const r=document.querySelector(o);r&&(r.value=t,r.dispatchEvent(new Event("input",{bubbles:!0})))}function K(o){for(const t of o||[])t.click?document.querySelector(t.click)?.click():t.fill?H(t.fill[0],t.fill[1]):t.clear?H(t.clear,""):t.clickText?[...document.querySelector(t.clickText[0])?.querySelectorAll("button")??[]].find(a=>a.textContent?.trim().includes(t.clickText[1]))?.click():t.key&&document.dispatchEvent(new KeyboardEvent("keydown",{key:t.key,bubbles:!0}))}const f=o=>String(o).replace(/[&<>]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[t]),U=o=>/^(#[0-9a-f]{3,8}|rgba?\([\d.,\s%/]+\)|hsla?\([\d.,\s%/]+\))$/i.test(String(o).trim());function G(o){return o.replace(/("[^"]*"|'[^']*')/g,'<span class="s">$1</span>').replace(/(var\()(--[\w-]+)/g,'$1<span class="t">$2</span>').replace(/(#[0-9a-fA-F]{3,8})\b/g,'<span class="n">$1</span>')}function J(o){return f(o).split(`
...
`)}function O(o){return f(o).replace(/("[^"]*")/g,'<span class="s">$1</span>').replace(/(&lt;\/?)([a-zA-Z][\w-]*)/g,'$1<span class="tag">$2</span>')}function Y(o){if(!o||!o.length)return'<p class="hint">No design tokens in this section.</p>';const t={};for(const a of o)(t[a.tier]=t[a.tier]||[]).push(a);return["brand","semantic","component","primitive"].filter(a=>t[a]).map(a=>`
      <div class="tgroup">
  ...
  .x:hover { color: #fff; }
`,P="handoff-inspector";function X(){const o="/biochar-design/";let t=location.pathname;return t.startsWith(o)&&(t=t.slice(o.length)),t=t.replace(/^\/|\/$/g,""),t.replace(/\//g,"-")||"index"}const Z=o=>`/biochar-design/handoff/${o}/manifest.json`;function Q(){if(document.getElementById(P))return;const o=Z(X());fetch(o).then(t=>t.ok?t.json():Promise.reject()).then(t=>ee(t,o)).catch(()=>{})}function ee(o,t){const r=document.createElement("div");r.id=P;const a=r.attachShadow({mode:"open"});document.documentElement.appendChild(r);const c=document.createElement("style");c.textContent=`
    ...
    </div>`,a.append($,s);const S=s.querySelector(".launch"),_=s.querySelector(".panel"),C=s.querySelector(".sub"),m=s.querySelector(".picker"),u=s.querySelector(".body"),T=[...s.querySelectorAll(".tabs button[data-tab]")],b=s.querySelector(".copy"),A=s.querySelector(".claude"),y=s.querySelector(".cpreview"),B=s.querySelector(".cpreview pre"),v=s.querySelector(".cpreview__copy"),j=()=>document.querySelectorAll("[data-handoff-on]").forEach(e=>e.removeAttribute("data-handoff-on")),L=e=>{j(),e&&document.querySelector(e)?.setAttribute("data-handoff-on","")};let n=null,l="guide",h=!1,k=!1;function q(){if(b.textContent=`Copy ${l}`,!n){C.textContent=`${o.name} · ${o.sections.length} sections`,u.innerHTML='<p class="hint">Pick a section above to inspect its markup, styles, behavior, tokens, and design intent.</p>';return}C.textContent=n.tag&&n.tag!=="page"?`${n.label} · <${n.tag}>`:n.label,l==="html"?u.innerHTML=`<pre class="code">${O(n.html)}</pre>`:l==="css"?u.innerHTML=n.css?`<pre class="code">${J(n.css)}</pre>`:'<p class="hint">No section-local CSS (inherited utilities only).</p>':l==="js"?u.innerHTML=n.js?`<pre class="code">${R(n.js)}</pre>`:'<p class="hint">No interactivity — this section is static markup.</p>':l==="tokens"?u.innerHTML=Y(n.tokens):u.innerHTML=V(n.guide)}const D=()=>[...m.querySelectorAll(".chips .chip")];function E(e,i=!1){if(n=o.sections[e]||null,D().forEach((d,p)=>d.classList.toggle("on",p===e)),w(),i&&h&&n?.apply){k=!0;const d=document.querySelector("[data-omni]");d&&!d.hasAttribute("hidden")&&document.querySelector("[data-omni-close]")?.click(),K(n.apply),k=!1}h&&L(n?.selector),q()}function N(){m.innerHTML='<div class="chips"></div>';const e=m.querySelector(".chips");o.sections.forEach((i,d)=>{const p=document.createElement("button");p.className="chip",p.title=i.apply?`${i.label} — click to drive the app into this state`:i.label,p.textContent=i.label,p.onclick=()=>E(d,!0),e.appendChild(p)})}const M=e=>{!h||k||e.composedPath().includes(_)||g(!1)};function g(e){h=e,_.classList.toggle("is-open",e),S.hidden=e,e?(document.head.append(c),L(n?.selector),setTimeout(()=>document.addEventListener("click",M,!0),0)):(c.remove(),j(),document.removeEventListener("click",M,!0))}function F(){if(!n)return"";if(l==="html")return n.html||"";if(l==="css")return n.css||"";if(l==="js")return n.js||"";if(l==="tokens")return(n.tokens||[]).map(i=>`${i.name}: ${i.value};`).join(`
...
`)}const x=(e,i,d=!0)=>{const p=e.innerHTML;e.classList.toggle("done",d),e.textContent=i,setTimeout(()=>{e.innerHTML=p,e.classList.remove("done")},1300)};b.onclick=async()=>{const e=F();if(e)try{await navigator.clipboard.writeText(e),x(b,"Copied")}catch{x(b,"Failed",!1)}};function z(){if(!n?.claudePath)return"";const e=new URL(I+n.claudePath,location.origin).href,i=[`Here's a new UI section to build — "${n.label}".`,"","The linked spec has the design guidance (intent, key decisions, gotchas) plus sample","HTML, CSS, and JS. The finished UI should look and behave exactly like this — match it","faithfully. The sample code shows how it's built; you don't need to mirror it","line-for-line — translate it to your own stack and design system, mapping the","sample's values onto your established tokens.","","Spec — use whichever you can reach:",`• hosted: ${e}`];return n.repoPath&&i.push(`• in this repo: ${n.repoPath}`),i.join(`
`)}const w=()=>y.hidden=!0;A.onclick=()=>{if(n?.claudePath){if(!y.hidden)return w();B.textContent=z(),y.hidden=!1}},v.onclick=async()=>{try{await navigator.clipboard.writeText(z()),x(v,"Copied ✓")}catch{x(v,"Failed",!1)}},S.onclick=()=>g(!0),s.querySelector(".x").onclick=()=>g(!1),T.forEach(e=>e.onclick=()=>{l=e.dataset.tab,T.forEach(i=>i.classList.toggle("on",i===e)),w(),q()}),document.addEventListener("keydown",e=>{e.altKey&&e.shiftKey&&(e.key==="I"||e.key==="i")?(e.preventDefault(),g(!h)):e.key==="Escape"&&h&&(e.preventDefault(),g(!1))}),N(),E(0),new URLSearchParams(location.search).has("inspect")&&g(!0)}Q();
